"""Workflow CLI handlers — ``mocode workflow <action>`` entry point.

Also exposes shared utilities (``make_registry``, ``parse_kv_args``) used by
both the top-level CLI and the REPL ``/workflow`` command.
"""
...
from .models import NodeResult, detailed_summarize
...
def _run_foreground(args) -> None:
    ...
    name = args.name
    user_args = parse_kv_args(args.kv_args)
    ...
    print(detailed_summarize(wf.name, results))
...
def _result(args) -> None:
    ...
    if results:
        print(detailed_summarize(name, results, max_lines=9999))
    else:
...
def _runs(args) -> None:
    ...
    for r in runs:
        ...
        print(f"  {rid}  {rname:20s}  {status}{alive_tag}  {started}")
