"""Connect command — provider management (add, edit, delete)."""
...
class ConnectCommand:
    name = "/connect"
    description = "Manage providers (add, edit, delete)"
    aliases = ()
    ...
    async def run(self, ctx: CommandContext) -> CommandResult:
        ...
        if chosen == "__add__":
            await self._add(ctx)
            return CommandResult.CONTINUE
        await self._edit(ctx, chosen)
        return CommandResult.CONTINUE

    async def _edit(self, ctx: CommandContext, key: str):
        ...
        while True:
            ...
            key_masked = mask_key(entry.api_key)
            ...
            if chosen == "name":
                ...
                await self._extra_body(ctx, entry)
                dirty = True
    ...
    async def _extra_body(self, ctx: CommandContext, entry: ProviderEntry):
    ...
    async def _add(self, ctx: CommandContext):
