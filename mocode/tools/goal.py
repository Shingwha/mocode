...
_GOAL_DESC = (
    "Manage a goal condition that the agent works toward. "
    "Actions: set (create goal), pause (temporarily stop), "
    "resume (continue paused goal), status (check state), clear (remove goal)."
)
...
class GoalTool(Tool):
    ...
    async def _execute(self, args: dict) -> str:
        ...
        if action == "resume":
            ...
            return (
                f"Goal resumed: {self._hook.condition} "
                f"(turn counter reset, {self._hook.max_turns} turns remaining)"
            )
