    ...
    def _execute(self, args: dict) -> str:
        ...
        if _is_vfs_path(pattern) or _is_vfs_path(base_path):
            ...
            if prefix != _VFS_PREFIX:
                ...
                keys = list(self._vfs)
            vfs_files = sorted(p for p in keys if fnmatch.fnmatch(_strip_prefix(p), clean_pattern))
            if not vfs_files:
        ...
        files = sorted(
            (p for p in base.glob(pattern) if p.is_file() and not any(part in IGNORE_DIRS for part in p.relative_to(base).parts)),
