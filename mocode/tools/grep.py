...
from .utils import (
    _get_type_filter, _is_vfs_path, _match_vfs_type, _walk_text_files,
    expand_context_indices, format_grep_content, format_grep_count, format_grep_files, require_dir,
)
...
def _search_files(pattern: re.Pattern, file_iter: Iterable[tuple[str, list[str]]],
                  output_mode: str, context_lines: int, max_results: int, not_found_msg: str) -> str:
    ...
    for display_path, file_lines in file_iter:
        ...
        result = format_grep_content(file_lines, match_indices, display_indices,
...
def _grep_single_file(pattern: re.Pattern, filepath: Path, output_mode: str, context_lines: int, max_results: int) -> str:
    try:
        lines = filepath.read_text(encoding="utf-8", errors="replace").splitlines()
    except Exception:
        return f"Cannot read file: {filepath}"
    return _search_files(pattern, [(str(filepath), lines)], output_mode, context_lines, max_results,
                         f"No matches for '{pattern.pattern}' in {filepath}")


def _grep_vfs(pattern: re.Pattern, vfs: VirtualFS, base_path: str,
              type_filter: set[str] | None, output_mode: str, max_results: int, context_lines: int) -> str:
    ...
    return _search_files(pattern, _iter(), output_mode, context_lines, max_results,
                         f"No matches for '{pattern.pattern}' in virtual files")


def _grep_real_fs(pattern: re.Pattern, base_path: Path,
                  type_filter: set[str] | None, output_mode: str, max_results: int, context_lines: int) -> str:
    cwd = Path.cwd()
    def _iter() -> Iterable[tuple[str, list[str]]]:
        for filepath in _walk_text_files(base_path, type_filter):
            try:
                ...
                continue
            dp = str(Path(filepath).relative_to(cwd)) if Path(filepath).is_relative_to(cwd) else filepath
            yield dp, lines
    return _search_files(pattern, _iter(), output_mode, context_lines, max_results,
...
_GREP_PARAMS = {
    ...
    "path": {"type": "string", "description": "Directory to search in (defaults to current directory). Also accepts a single file path. Use 'vfs://' to search virtual files only.", "default": "."},
    ...
    "ignore_case": {"type": "boolean", "description": "Case-insensitive matching (default false)", "default": False},
}
_GREP_DESC = (
...
class GrepTool(Tool):
    ...
    def _execute(self, args: dict) -> str:
        ...
        if _is_vfs_path(base_path):
            ...
            return _grep_vfs(pattern, self._vfs, base_path, type_filter, output_mode, max_results, context_lines)
