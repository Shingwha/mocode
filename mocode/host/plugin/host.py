...
class PluginHost:
    ...
    def __init__(
        ...
        *,
        ...
        #: Plugins the caller brings along — normally a frontend's own.
    ...
    def load(self) -> list[Plugin]:
        ...
        reserved = {p.name for p in builtins} | {p.name for p in self.extra_plugins}
        self.plugins = [p for p in builtins if self._enabled(p.name, True)]
        self.plugins.extend(p for p in self.extra_plugins if self._enabled(p.name, True))
