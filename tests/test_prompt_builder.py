...
from mocode.core.prompt.builder import PromptBuilder, PromptContributions, Section
...
class TestPromptBuilder:
    ...
    def test_empty_render_skipped(self):
        ...
        # Empty section should not add extra newlines
        assert result == "Here"

    def test_multiple_sections(self):
        builder = PromptBuilder()
        builder.add(Section(name="a", priority=10, render=lambda ctx: "A"))
        builder.add(Section(name="b", priority=20, render=lambda ctx: "B"))
        builder.add(Section(name="c", priority=30, render=lambda ctx: "C"))
        result = builder.build()
        assert "A" in result
        assert "B" in result
        assert "C" in result
